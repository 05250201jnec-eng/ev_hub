 ...
 *   GPIO 18 → Reed Switch / Slide Switch (other leg to GND, uses INPUT_PULLUP)
 ...
 *   (SocketIoClient is NOT used — it has a Wokwi build bug)
 */
...
// ▼ FOR REAL HARDWARE: comment the above, uncomment & fill these
...
// ── WebSocket client (raw — no SocketIoClient library needed) ─────────────────
WebSocketsClient ws;
bool wsConnected = false;

// ── Emit a Socket.IO event ────────────────────────────────────────────────────
// Socket.IO over raw WS format: 42["event_name", {"key": "value"}]
void emitEvent(const char* eventName, const char* stationId) {
  String msg = "42[\"";
  msg += eventName;
  msg += "\",{\"stationId\":\"";
  msg += stationId;
...
void setHardwareState() {
  ...
  switch (state) {
    case OFFLINE:
      ...
      if (now - lastBlink > 1000) {      // Slow red blink = offline
        lastBlink = now;
        blinkOn = !blinkOn;
...
void wsEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    ...
    case WStype_TEXT: {
      ...
      // Socket.IO namespace connected → station is now AVAILABLE
      if (msg.startsWith("40")) {
        Serial.println("[Socket.IO] ✓ Connected to namespace — Station AVAILABLE");
        state = AVAILABLE;
        break;
      }
...
void loop() {
  ...
  if ((millis() - lastDebounce) > DEBOUNCE_MS) {
    // LOW = closed = magnet present = gun plugged in
    if (reading == LOW && !pluggedIn) {
      pluggedIn = true;
      Serial.println("[Reed] ▼ Gun PLUGGED IN — emitting charger_plugged");
      emitEvent("charger_plugged", STATION_ID);
      state = CHARGING;
    }
    // HIGH = open = magnet gone = gun unplugged
    else if (reading == HIGH && pluggedIn) {
      pluggedIn = false;
      Serial.println("[Reed] ▲ Gun UNPLUGGED — emitting charger_unplugged");

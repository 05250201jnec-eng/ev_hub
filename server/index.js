...
io.on('connection', (socket) => {
  ...
  // ─── ESP32 Physical Charger Handlers ──────────────────────────────────────────
  socket.on('charger_plugged', async ({ stationId }) => {
    console.log(`[ESP32] Physical plug-in detected at station: ${stationId}`);
    if (!stationState[stationId]) return;

    try {
      let userId = 'guest';
      let userName = 'EV Driver';
      let stationName = stationState[stationId].name;

      if (db) {
        // Fetch the pending plug_in user who scanned the QR code
        const docRef = db.collection('stations').doc(stationId);
        const docSnap = await docRef.get();
        if (docSnap.exists) {
          const data = docSnap.data();
          userId = data.plugInUser || 'guest';
          userName = data.plugInUserName || 'EV Driver';
          
          // Update station status to charging
          await docRef.update({
            status: 'charging',
            plugInUser: null, // Clear pending state
            plugInUserName: null,
            lastUpdated: admin.firestore.FieldValue.serverTimestamp(),
          });
        }
      ...
      // Start simulated transaction & status update
      simulateStartTransaction(stationId);
      simulateStatusChange(stationId, 'charging');

      // Create active session document in Firestore to instantly notify User App
      if (db) {
        const sessionId = stationState[stationId].sessionId || `sess-${stationId}-${Date.now()}`;
      ...
      console.log(`[ESP32] Charging session activated successfully for user: ${userName}`);
    } catch (err) {
      console.error('[ESP32] Error handling charger_plugged:', err.message);
    }
  });

  socket.on('charger_unplugged', async ({ stationId }) => {
    console.log(`[ESP32] Physical unplug detected at station: ${stationId}`);
    if (!stationState[stationId]) return;

    try {
      const sessionId = stationState[stationId].sessionId;
      
      // Update completed session in Firestore
      if (db && sessionId) {
      ...
      console.log(`[ESP32] Charging session stopped successfully.`);
    } catch (err) {
      console.error('[ESP32] Error handling charger_unplugged:', err.message);
    }
  });

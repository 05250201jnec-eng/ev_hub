...
const MapPage = () => {
  ...
  // Dynamically load Google Maps JavaScript API for routing
  useEffect(() => {
  ...
  const handleDirections = async (station) => {
    ...
    if (!window.google || !window.google.maps) {
      addNotification("Google Maps Service is loading, please try again in a moment.", "info");
      return;
    }
    ...
    try {
      ...
      directionsService.route(
        {
          origin: new window.google.maps.LatLng(userLocation[0], userLocation[1]),
        ...
        (result, status) => {
          ...
          if (status === window.google.maps.DirectionsStatus.OK) {
            ...
            // Convert Google Maps path coordinates to Leaflet latlng format
            ...
            addNotification("Could not calculate driving route with Google Maps.", "warning");
          }
        }
      );
    } catch (e) {

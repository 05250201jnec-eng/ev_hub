...
const Session = () => {
  const { bookings, activeSession } = useAppContext();
  const navigate = useNavigate();

  // Case 1: IoT-triggered session from the sessions collection (ESP32 physical plug-in)
  if (activeSession) {
    return (
      <div className="animate-fade-in">
        <h1 style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '2rem' }}>Active Session</h1>
        <LiveChargingSession booking={activeSession} />
      </div>
    );
  }
  ...
  // Case 3: No session — prompt to scan QR
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', color: 'var(--text-secondary)' }}>
      ...
      <p style={{ marginBottom: '2rem', textAlign: 'center', maxWidth: 280 }}>
        Scan the QR code at an EV Hub station to start charging.
      </p>
